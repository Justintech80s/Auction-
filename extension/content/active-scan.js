export function collectActiveProductEvidence(options = {}) {
  ...
  function httpsUrl(value) {
    ...
    try {
      const url = new URL(value.trim(), locationLike?.href);
  ...
  function attr(selector, attribute = 'content') {
    const element = documentLike?.querySelector?.(selector);
    const value = element?.getAttribute?.(attribute);
    return cleanText(value, attribute === 'content' ? 2048 : 512);
  }
  ...
  function firstProductJsonLd() {
    ...
    for (const script of scripts) {
      ...
      try {
        ...
        // Ignore malformed page-controlled structured data.
      }
    }
    return null;
  }
  ...
  function brandName(value) {
    ...
    if (value && typeof value === 'object') return cleanText(value.name, 128);
    return null;
  }
  ...
  function conditionName(value) {
    ...
    return cleanText(tail || text, 128);
  }

  function firstImage(value) {
    if (Array.isArray(value)) return value.map(firstImage).find(Boolean) ?? null;
    if (value && typeof value === 'object') return httpsUrl(value.url || value.contentUrl);
    return httpsUrl(value);
  }

  function addRecord(target, key, value, maxEntries) {
    if (Object.keys(target).length >= maxEntries) return;
    const cleanKey = cleanText(key, 64);
    const cleanValue = cleanText(value, 256);
    if (cleanKey && cleanValue) target[cleanKey] = cleanValue;
  }
  ...
  if (structured) {
    evidenceKinds.push('structured_product');
    confidence = Math.max(confidence, 0.70);
    title = cleanText(structured.name, 512);
    brand = brandName(structured.brand);
    model = cleanText(structured.model, 128);
    category = cleanText(structured.category, 256);
    condition = conditionName(structured.itemCondition);
    imageUrl = firstImage(structured.image);
    for (const key of ['sku', 'mpn', 'gtin', 'gtin8', 'gtin12', 'gtin13', 'gtin14']) {
      addRecord(identifiers, key, structured[key], 20);
    }
  }
  ...
  const metaImage = attr('meta[property="og:image"]') || attr('meta[name="twitter:image"]');
  if (!imageUrl && metaImage) imageUrl = httpsUrl(metaImage);

  const metaModel = attr('meta[itemprop="model"]');
  if (!model && metaModel) model = cleanText(metaModel, 128);

  for (const [selector, key] of [
    ['meta[itemprop="sku"]', 'sku'],
    ['meta[itemprop="mpn"]', 'mpn'],
    ['meta[itemprop="gtin"]', 'gtin']
  ]) {
    const value = attr(selector);
    if (value) addRecord(identifiers, key, value, 20);
  }

  const pageTitle = cleanText(documentLike?.title, 512);
  ...
  if (model || Object.keys(identifiers).length > 0) {
    confidence = Math.min(1, confidence + 0.20);
    if (!evidenceKinds.includes('strong_identifiers')) evidenceKinds.push('strong_identifiers');
  }

  if (imageUrl) {
    confidence = Math.min(1, confidence + 0.10);
    if (!evidenceKinds.includes('primary_image')) evidenceKinds.push('primary_image');
  }
