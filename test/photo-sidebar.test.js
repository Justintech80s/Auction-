...
test('sidebar accepts image files and sends photo data to Auction API',()=>{const src=read('extension/sidepanel/shared-session.js');assert.match(src,/readAsDataURL/);assert.match(src,/browser_extension_photo/);assert.match(src,/image:\{dataUrl:selected\.dataUrl/);assert.match(src,/api\/product-scan/);});
