...
import { applyCostPreset, initializeDefaultCostPreset } from '../extension/sidepanel/app.js';
...
function fakeDocument(values = {}) {
  ...
  for (const [id, value] of Object.entries(values)) elements.set(id, { value: String(value), textContent: '' });
  return {
    getElementById(id) {
...
test('cost preset store saves only explicit allowlisted non-negative costs', async () => {
  ...
  const store = createCostPresetStore(storage, { now: () => new Date('2026-09-13T02:00:00.000Z') });
  const saved = await store.save('eBay resale', { marketplaceFee: 18.25, shipping: 12, tax: 0, repairs: 5, paymentProcessing: 3.5, holding: 1, secret: 'drop-me' });
  assert.deepEqual(saved.costs, { marketplaceFee: 18.25, shipping: 12, tax: 0, repairs: 5, paymentProcessing: 3.5, holding: 1 });
...
test('cost preset store overwrites the same normalized name and lists newest first', async () => {
  let tick = 0;
  const storage = fakeStorage();
  const store = createCostPresetStore(storage, { now: () => new Date(`2026-09-13T02:00:0${tick++}.000Z`) });
...
test('cost preset store removes presets and enforces the 20 preset cap', async () => {
  let tick = 0;
  const storage = fakeStorage();
  const store = createCostPresetStore(storage, { now: () => new Date(1_800_000_000_000 + tick++ * 1000) });
  for (let index = 0; index < MAX_COST_PRESETS + 2; index += 1) await store.save(`Preset ${index}`, { shipping: index });
  assert.equal((await store.list()).length, MAX_COST_PRESETS);
  assert.equal(await store.remove('preset 21'), true);
});

test('malformed persisted cost presets are cleaned instead of trusted', async () => {
  const storage = fakeStorage({ [COST_PRESET_STORAGE_KEY]: [
  ...
  assert.deepEqual((await store.list()).map(p => p.name), ['Good']);
});
...
test('default preset initialization auto-applies only the stored explicit costs', async () => {
  const storage = fakeStorage();
  const store = createCostPresetStore(storage);
  const preset = await store.save('Daily resale', { marketplaceFee: 11, shipping: 7 });
  await store.setDefault(preset.key);
  const documentLike = fakeDocument({ 'cost-marketplace-fee': '99', 'cost-shipping': '99', 'cost-tax': '99', 'cost-preset-name': '' });
  const applied = await initializeDefaultCostPreset(documentLike, store);
  assert.equal(applied.key, preset.key);
  assert.equal(documentLike.getElementById('cost-marketplace-fee').value, '11');
  assert.equal(documentLike.getElementById('cost-shipping').value, '7');
  assert.equal(documentLike.getElementById('cost-tax').value, '');
  assert.equal(documentLike.getElementById('cost-preset-name').value, 'Daily resale');
});

test('applying a cost preset populates only the six existing explicit cost fields', () => {
  const documentLike = fakeDocument({ unrelated: 'leave-me' });
  applyCostPreset(documentLike, { marketplaceFee: 12.5, shipping: 8, repairs: 0 });
...
test('side panel markup exposes preset name, select, save, apply, delete, and set-default controls', async () => {
  ...
  for (const id of ['cost-preset-name','cost-preset-select','save-cost-preset','apply-cost-preset','delete-cost-preset','set-default-cost-preset']) {
