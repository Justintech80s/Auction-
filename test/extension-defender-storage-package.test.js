...
test('release replaces Defender-flagged storage modules with consolidated browser storage', async () => {
  ...
  assert.equal(await exists('dist/extension/storage/watchlist.js'), false);
  assert.equal(await exists('dist/extension/storage/cost-presets.js'), false);
  assert.equal(await exists('dist/extension/storage/browser-storage.js'), true);
});
